...
print ("digraph "+filename+" {") # graph starts
print ("  rankdir=LR;")
print ("  graph [nodesep=.7, rankdir=LR, splines=ortho];")
print ("  node [shape=record, width=1.5, height=.1];")
print ("  legenda [label = \"{ES|D|EF} | {Task} | {LF|S|LF}\"]", end="\n\n") # legend record

maxEF=datetime.date(2020, 1, 1)
...
print ("  legenda2 [label = \"{temporary: don't read} | {", sep="", end="")
print (maxEF.strftime("%d/%m/%Y"), sep="", end="")
print ("} | {# comment}\"]", sep="")
